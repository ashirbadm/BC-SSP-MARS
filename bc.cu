...
int main(int argc, char *argv[]) {
	unsigned *nodesigma, *edgesrc, *edgedst, *nodedist, *edgewt,*psrc,*noutgoing,*edgesigma,*border,*nerr, *child_count;
...
//	source = 1462550;
	...
	if(s_count_cpu < num_srcs){
		s_count_cpu++;
		while(1){
		source = 12;//rand() % graph.nnodes;//vis_srcs[iter];//rand() % graph.nnodes;
