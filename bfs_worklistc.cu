...
__device__
unsigned processnode2(foru *dist, unsigned hnodes,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc,unsigned *edgesdst,unsigned *noutgoing , Worklist &inwl, Worklist &outwl, unsigned iteration, unsigned *border, unsigned *child_count, int *child) 
{
...
*/
__device__
void drelax(foru *dist, unsigned hnodes, unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist &inwl, Worklist& outwl, int iteration, unsigned *border, unsigned *child_count, int *child) {
	//unsigned id = blockIdx.x * blockDim.x + threadIdx.x;
/*
	if(iteration == 0)
	  {
	  ...
	  {
	 */
	    if(processnode2(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,inwl, outwl, iteration, border, child_count, child))
	      *gerrno = 1;
	  //}
}
...
__global__ void drelax2(foru *dist, unsigned hnodes,int *d_sources,unsigned source_count,unsigned *nodesigma,unsigned *edgesigma,unsigned *srcsrc, unsigned *edgesdst,unsigned *noutgoing,unsigned *gerrno, Worklist inwl, Worklist outwl, int iteration, GlobalBarrier gb, unsigned *border, int *d_source_sigma, int *flag, unsigned *child_count, int *child)
{
	unsigned processed_count = 0;
	Worklist *in;
        Worklist *out;
        Worklist *tmp;
	iteration = -d_sources[0];
...
//		printf("[GPU] Before loop\n");
	...
	in = &inwl; out = &outwl;
	//if (threadIdx.x == 0 && blockIdx.x == 0) printf("[GPU4]  iteration = %d, *in->dindex = %d\n",iteration, *in->dindex);
	while(*in->dindex > 0 )//|| d_sources[processed_count]!=-100000) // && iteration < 30)
        {
		iteration++;
//	if(*in->dindex > 0)
		drelax(dist, hnodes,nodesigma,edgesigma,srcsrc, edgesdst,noutgoing,gerrno, *in, *out, iteration, border, child_count, child);
	...
	//	if (threadIdx.x == 0 && blockIdx.x == 0) printf("[GPU4] iteration = %d\n",iteration);
		...
		//	if(processed_count+1 < source_count)
				processed_count = addSourcesOnCurrentLevel(dist, d_sources, processed_count, *out, iteration, d_source_sigma, nodesigma, flag);
        	}
		gb.Sync();
	//	if (threadIdx.x == 0 && blockIdx.x == 0) 
...
//	outwl->display_items();
	...
	drelax2 <<<13 * drelax2_max_blocks, BLKSIZE,0, *s_multi>>>(dist, hnodes,d_sources,source_count,nodesigma,edgesigma,d_edgessrc,d_edgesdst,d_noutgoing, nerr, *inwl, *outwl, 0, gb,d_border,d_sigma, flag, child_count, child);
